            ...
            <div class="wrap-form">
             <form class="form-info" id="form0">
                    ...
                    <label class="">
                        ...
                        <div>
                            <input id="4gip" name="4gip" type="text" value="<% [ `ubus call network.interface.wan1 status |grep "\"address\":" |cut -d: -f2 |tr -d "\"\, "` ] && ubus call network.interface.wan1 status |grep "\"address\":" |cut -d: -f2 |tr -d "\"\, " || echo "$g4_noip" %>" readonly="readonly" style="background-color:#eee" />
                        </div>
                    </label>
                    <label class="">
                        <div class="name"><%= $dialmode%>:</div>
                        <select class="dialmode" name="dialmode" onchange="mode_change()">
                          <option value="gobinet" <% [ `uci get network.wan1.proto |grep "dhcp"` ] && echo 'selected="true"' %> >Gobinet</option>
                          <option value="pppd" <% [ `uci get network.wan1.proto |grep "3g"` ] && echo 'selected="true"' %> >PPPD</option>
                        </select>
                    </label>
					...
					if [ "`uci get network.wan1.device`" = "$dev" ];then
                        ...
                        <div>
                            <select class="at" name="at">
                                ...
                                for dev in `ls /dev/tty[U,A][S,C][B,M]*`
                                  do
                                    echo -n "<option value=\"$dev\" "
					if [ "`uci get 4g.modem1.device`" = "$dev" ];then
                                         echo "selected="true">$dev</option>"
					else
			            	echo  ">$dev</option>"
					fi
              ...
              <select class="apn" name="apn" onchange="mask_method()">
								<option value="3gnet" <% [ `uci get network.wan1.apn |grep "3gnet"` ] && echo 'selected="true"' %> ><%= $g3net%></option>
								<option value="ctnet" <% [ `uci get network.wan1.apn |grep "ctnet"` ] && echo 'selected="true"' %> ><%= $ctnet%></option>
                    ...
                    <label class="">
                        ...
                        <div>
                            <input id="dialnumber" name="dialnumber" type="text" value="<% uci get network.wan1.dialnumber %>" placeholder="*99#" />
                        </div>
                    </label>
                    ...
                    <label class="">
                        ...
                        <div>
                            <input id="username" name="username" type="text" value="<% uci get network.wan1.username %>" placeholder="<%= $place_hold%>" />
                        </div>
                    </label>
                    <label class="">
                        <div class="name"><%= $passwd%></div>
                        <div>
                            <input id="password" name="password" type="text" value="<% uci get network.wan1.password %>"  placeholder="<%= $place_hold%>" />
                        </div>
                    </label>
                    <label class="">
                        <div class="name"><%= $pincode%></div>
                        <div>
                            <input id="pincode" name="pincode" type="text" value="<% uci get network.wan1.pincode %>" placeholder="<%= $place_hold%>" />
                        </div>
                    </label>
                    <input  name="modemid" type="hidden" value="1" />
            </form>
				  <div class="btn-wrap">
